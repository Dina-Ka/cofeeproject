...
AUTH0_DOMAIN = 'project-three-fsnd.us.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'drinks'
...
def verify_decode_jwt(token):
    ...
    if 'kid' not in unverified_header:
        raise AuthError({
            'code': 'invalid_header',
            'description': 'Authorization not formed.'
        }, 401)
    ...
    if rsa_key:
        try:
            ...
            raise AuthError({
                'code': 'token_expired',
                'description': 'unfortunatelly the token is expired.'
            }, 401)
        except jwt.JWTClaimsError:
            raise AuthError({
                'code': 'invalid_claims',
                'description': 'error in JWTClaimsError'
            }, 401)
        except Exception:
            raise AuthError({
                'code': 'invalid_header',
                'description': 'cant parse authentication token.'
            }, 400)
    raise AuthError({
        'code': 'invalid_header',
